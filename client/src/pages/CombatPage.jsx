      ...
      <div className="relative flex flex-col md:flex-row gap-8 items-center justify-between mb-12 min-h-[300px]">
          ...
          {isCoop ? (
            ...
            <div className={`panel w-full max-w-sm bg-dark-surface/80 border-dark-border text-center ${playerShakes['me'] ? 'animate-shake' : ''}`}>
              <div className="h-32 md:h-48 mb-4 flex items-center justify-center">
                {(!myPlayerState.isAlive && CLASS_INFO[myPlayerState.class]?.sprites?.dead) ? <img src={CLASS_INFO[myPlayerState.class].sprites.dead} alt={myPlayerState.name} className="max-w-full max-h-full object-contain [image-rendering:pixelated]" /> : ((CLASS_INFO[myPlayerState.class]?.sprites?.idle || CLASS_INFO[myPlayerState.class]?.sprite) ? <img src={(isMyTurn && enemyShake) ? (CLASS_INFO[myPlayerState.class]?.sprites?.attack || CLASS_INFO[myPlayerState.class]?.sprites?.idle || CLASS_INFO[myPlayerState.class]?.sprite) : (CLASS_INFO[myPlayerState.class]?.sprites?.idle || CLASS_INFO[myPlayerState.class]?.sprite)} alt={myPlayerState.name} className="max-w-full max-h-full object-contain drop-shadow-lg [image-rendering:pixelated]" /> : <span className="text-6xl">{CLASS_INFO[myPlayerState.class]?.icon}</span>)}
              </div>
        ...
        <div className={`flex-1 w-full max-w-sm panel bg-dark-surface/80 border-dark-border text-center ${enemyShake ? 'animate-shake' : ''}`}>
          <div className="h-32 md:h-48 mb-4 flex items-center justify-center drop-shadow-[0_0_15px_rgba(196,75,47,0.4)]">
            {(enemy.sprites?.idle || enemy.sprite) ? <img src={(enemy.hp <= 0 && enemy.sprites?.dead) ? enemy.sprites.dead : (playerShakes['me'] ? (enemy.sprites?.attack || enemy.sprites?.idle || enemy.sprite) : (enemy.sprites?.idle || enemy.sprite))} alt={enemy.name} className="max-w-full max-h-full object-contain [image-rendering:pixelated]" /> : <span className="text-6xl">{enemy.isBoss ? '👹' : '💀'}</span>}
          </div>
